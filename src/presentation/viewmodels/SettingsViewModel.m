classdef SettingsViewModel < handle
    ...
    methods
        ...
        function openDefaultsDialog(obj)
            ...
            % Default mitigation level (Phase 3.6). Mirrors the
            % Cutting toolbar's mitigation dropdown so the operator's
            % chosen ladder level persists across sessions. The
            % Cutting screen's CuttingMitigationDropdown picks this up
            % from app.State.preferredMitigationLevel at first show.
            SettingsViewModel.addFormLabel(g, 3, 1, ...
                Labels.get('settings_label_mitigation_level', ...
                           'Default mitigation level'));
            app.SettingsMitigationLevelDropdown = uidropdown(g, ...
                'Items', { ...
                    'Raw', ...
                    'Standard (default)', ...
                    'Aggressive', ...
                    ...
                    'Custom (advanced)'}, ...
                'ItemsData', {0, 1, 2, 3, -1}, ...
                'Value', double(obj.Values.defaultMitigationLevel));
            app.SettingsMitigationLevelDropdown.Layout.Row = 3;
            app.SettingsMitigationLevelDropdown.Layout.Column = 2;
            app.SettingsMitigationLevelDropdown.Tooltip = ...
                ['Default ladder level applied to every new submission. ' ...
                 'The Cutting screen toolbar dropdown initialises to ' ...
                 'this preference; per-submission override still ' ...
                 'available there.'];

            SettingsViewModel.addFormLabel(g, 4, 1, Labels.get('settings_label_timeout'));
            app.SettingsTimeoutField = uieditfield(g, 'numeric', 'Value', obj.Values.timeoutSec);
            app.SettingsTimeoutField.Layout.Row = 4; app.SettingsTimeoutField.Layout.Column = 2;

            SettingsViewModel.addFormLabel(g, 5, 1, Labels.get('settings_label_log_level'));
            ...
            app.SettingsLogLevelDropdown.Layout.Row = 5; app.SettingsLogLevelDropdown.Layout.Column = 2;
        ...
        function onSaveSettings(obj, dlg)
            ...
            % Phase 3.6: Default mitigation level dropdown.
            try
                if ~isempty(app.SettingsMitigationLevelDropdown) && ...
                        isvalid(app.SettingsMitigationLevelDropdown)
                    obj.Values.defaultMitigationLevel = ...
                        double(app.SettingsMitigationLevelDropdown.Value);
                end
            catch ME
                Logger.debug('SettingsViewModel', ...
                    'mitigation-level dropdown read: %s', ME.message);
            end
            ...
            app.State.preferredMitigationLevel = ...
                obj.Values.defaultMitigationLevel;
            app.logEvent('CONFIG', sprintf( ...
                'Session settings updated — shots: %d  opt: %d  mitig: %d', ...
            ...
            if app.State.isAuthenticated()
                ...
                prefs = struct( ...
                    ...
                    'default_mitigation_level', ...
                        int32(obj.Values.defaultMitigationLevel));
                svc   = app.SettingsSvc;
                token = app.State.authToken;
                AsyncRunner.run( ...
