...
function ReportsScreen(app)
    ...
    %  has a real intrinsic width and overflows narrow panels. The
    %  '1x' on Title made the column stretch to fit the panel
    %  exactly — meaning the table never overflowed, and uitable's
    %  horizontal scrollbar never engaged. With all-fixed widths
    %  the 4 columns total ~1160 px; if the panel is narrower the
    %  CEF scrollbar appears. Title 750 px also accommodates long
    %  report titles like "Quantum Monte Carlo VaR Report —
    %  cat_state_n22.qasm" without truncation. Vertical scrolling
    %  between rows is handled natively by uitable when row count
    %  exceeds viewport.
    %    Format 100, Title 750, Created 180, Status 130
    app.ReportsTable = uitable(lg, ...
        'ColumnName',    Labels.cols('reports_table_cols', ...
            {'Format', 'Title', 'Created', 'Status'}), ...
        'ColumnWidth',   {100, 750, 180, 130}, ...
        'RowName',       {}, ...
        'SelectionType', 'row', ...
        'Multiselect',   'off', ...
