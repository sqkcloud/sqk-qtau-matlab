...
function CircuitCuttingScreen(app)
    ...
    % ── Row 1: Toolbar ───────────────────────────────────────────────────
    %   [Circuit ▼] | [Mode ▼] | [Target k spinner] | (flex) |
    ...
    %  Mitigation level selector. Populated from GET /api/mitigation/
    %  levels at first show by CircuitCuttingViewModel.refreshMitigation
    %  Levels; this initial set is a hardcoded mirror of the server
    %  ladder so the toolbar is usable even when the levels endpoint
    %  is unreachable. ValueData carries the int level id (0..3, -1)
    %  that flows into CreateBatchRequest.mitigation_level at Run time.
    mitigLbl = uilabel(tb, 'Text', 'Mitigation', ...
        'FontSize', 12, 'FontColor', Theme.COLOR_LABEL, ...
        'HorizontalAlignment', 'right', 'VerticalAlignment', 'center');
    mitigLbl.Layout.Column = 8;

    %  Initial value comes from app.State.preferredMitigationLevel
    %  (Phase 3.6 — Settings tab persists the operator's chosen
    %  default across sessions). Falls back to 1 (Standard) when
    %  AppState lacks the field (older test fixtures, freshly-
    %  initialised app pre-login) or when the persisted value isn't
    %  one of the supported ladder ids.
    initialMitigLevel = 1;
    try
        cand = double(app.State.preferredMitigationLevel);
        if any(cand == [-1 0 1 2 3])
            initialMitigLevel = cand;
        end
    catch; end
    app.CuttingMitigationDropdown = uidropdown(tb, ...
        'Items', { ...
            'Raw', ...
            'Standard (default)', ...
            'Aggressive', ...
            ...
            'Custom (advanced)'}, ...
        'ItemsData', {0, 1, 2, 3, -1}, ...
        'Value', initialMitigLevel, ...
        'ValueChangedFcn', @(src,~) ...
            app.CircuitCuttingVm.onMitigationLevelChanged(src.Value));
    app.CuttingMitigationDropdown.Layout.Column = 9;
    app.CuttingMitigationDropdown.Tooltip = ...
        ['Mitigation level applied to every cutting subcircuit. ' ...
         'Standard = twirling + DD + TREX (free overhead, default). ' ...
         'Aggressive = +ZNE (~3x wall-clock). TEM routes to the ' ...
         'Algorithmiq IBM Function (Phase 4; falls back to Standard ' ...
         'today). Cost line below updates live as you change this.'];
    ...
    app.CuttingPresetDropdown.Layout.Column = 10;
    ...
    app.CuttingAnalyzeBtn.Layout.Column = 11;
    ...
    app.CuttingRunBtn.Layout.Column = 12;
    ...
    %  Inner grid — single row containing the action bar (CTA label
    %  + 4 navigation buttons). The state-header strip that lived in
    %  row 1 of the previous layout was removed because the toolbar
    %  above the panel already shows the same batch identity / status
    %  via setStatus(app.CuttingStatusLabel, ...) — keeping it inside
    %  the panel was duplicate information. Removing it pulls the
    %  action row ~26 px closer to the panel's top border.
    rg = uigridlayout(resPanel, [1 1]);
    rg.RowHeight = {36};
    rg.RowSpacing = 0;
    rg.Padding = [18 10 18 10]; rg.BackgroundColor = Theme.COLOR_CARD;

    % Action bar. Single-row layout — call-to-action label occupies
    % the flex column on the left; four navigation buttons (Jobs →
    % Results → View Reconstruction → Detailed Analysis) sit on the
    % right at widths matching the Results screen footer for visual
    % consistency. All buttons are gated by refreshActionButtons in
    % the VM, with educational disabled tooltips until the underlying
    % state is ready.
    ...
    % Hidden legacy host — keeps two widgets instantiated as
    % app.* properties so VM call-sites don't crash, but never
    % renders inline (parent Visible='off' wins over child Visible).
    %
    %   * CuttingResultsLabel — textarea the VM's renderResult
    %     populates after a successful batch. The View Reconstruction
    %     popup is the canonical surface for actual reconstructed
    %     values; this textarea is just a back-compat target.
    %   * CuttingActionsHeader — "Batch <id> · Status" line the
    %     M2 design rendered above the action row. Removed from the
    %     visible grid (the toolbar above the panel already shows
    %     the same status), retained here so refreshActionButtons
    %     can still set Text/Visible without checking for the widget.
    legacyHost = uipanel(rg, 'Title', '', 'BorderType', 'none', ...
        'Visible', 'off', 'BackgroundColor', Theme.COLOR_CARD);
    legacyHost.Layout.Row = 1;  % share cell with action row (Visible='off' wins)
    legacyHostGrid = uigridlayout(legacyHost, [2 1]);
    legacyHostGrid.RowHeight = {'fit', 'fit'};
    legacyHostGrid.Padding = [0 0 0 0];
    legacyHostGrid.BackgroundColor = Theme.COLOR_CARD;
    app.CuttingActionsHeader = uilabel(legacyHostGrid, ...
        'Text', '', ...
        'FontSize', 11, 'FontColor', Theme.COLOR_MUTED, ...
        'HorizontalAlignment', 'left', 'VerticalAlignment', 'center', ...
    ...
    %  Glyphs use BMP-range (≤U+FFFF) codepoints so MATLAB's uibutton
    %  font fallback resolves them on macOS. Surrogate-pair emoji
    %  (char(128xxx)) render as tofu boxes on macOS — see M11.1.
    %  Glyphs picked to match the same icons the Results screen
    %  action bar already uses for the same destinations.
