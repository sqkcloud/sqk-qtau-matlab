    ...
    methods (Static, Access = private)
        ...
        function manifest = buildManifest(tmpDir, files)
            ...
            manifest = struct( ...
                ...
                'generator',      AppConfig.get('app_name', 'QTAU: Hardware-Agnostic Execution'), ...
                'files',          entries);
        end
        ...
        function txt = buildReadme(ctx, opts, files)
            ...
            lines = {
                ...
                ''
                sprintf('Generated by %s on %s.', AppConfig.get('app_name', 'QTAU: Hardware-Agnostic Execution'), stamp)
                ''
                '## Contents'
                ''};
            for i = 1:numel(files)
