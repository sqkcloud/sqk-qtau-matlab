classdef ProjectService < handle
    ...
    methods
        ...
        function data = saveBenchmarkConfig(obj, projectId, shots, optLevel, mitigation, strategy, token)
            ep = sprintf('/api/projects/%s/benchmark-config', char(projectId));
            Logger.info('ProjectService', 'saveBenchmarkConfig → POST %s', ep);
            Logger.info('ProjectService', '  shots: %d  opt: %d  mitigation: %s  strategy: %s', ...
                round(shots), round(optLevel), char(mitigation), char(strategy));
            payload = struct( ...
                'shots',                  round(shots), ...
